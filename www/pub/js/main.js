...
$(function(){
	...
	// 세무 chat 등 외부 넘어가는 링크 class on 등의 잔상 제거
	var keep = $('.subNav_wrap .sub_nav li.on').first(); // 최초 on 보관
	$('.subNav_wrap .sub_nav')
	.on('click.skipFix', 'li.link-btn a, a[target="_blank"], a[data-skip-on]', function(){
		setTimeout(function(){
			$('.subNav_wrap .sub_nav li').removeClass('on');
			keep.addClass('on');
		}, 0);
	});
});
