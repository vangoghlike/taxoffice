...
<html>
    ...
    <body bgcolor="#FFFFFF" text="#242424" leftmargin=0 topmargin=15 marginwidth=0 marginheight=0 bottommargin=0 rightmargin=0>
        <div style="padding:10px;width:100%;font-size:14px;color: #ffffff;background-color: #ffffff;text-align: center">
            표준결제 인증결과 수신 / 승인요청, 승인결과 표시
        </div>
<?php
					...
					if ((strcmp("0000", $resultMap["resultCode"]) == 0) && (strcmp($secureSignature, $resultMap["authSignature"]) == 0) ){	//결제보안 추가 2016-05-18
				       ...
				       ******************************************************************************/
					   ...
					   //////////////20180604 make Jeejin////////////////////////ST
						...
						$OID = substr($TOID,strpos($TOID,"_")+1,strlen($TOID));
						$arrOID = explode("T",$OID);
						$idx = $arrOID[1];
						$pay_type = $resultMap["payMethod"];
						...
						if($arrInfo["list"][0]["save_point"] > 0){
							// 실 결제 금액에서 5% 적립
							$memInfo = getUserInfo($arrInfo["list"][0]["user_id"]);
