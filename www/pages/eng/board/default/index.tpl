...
{ CONTENTS['head_contents'] }
					...
					<div class="side">
						...
						<div class="right">
							<div class="searchArea">
								...
								{ ? _GET.ord }<a href="#" class="btn_box green btn_ord init">In basic order</a>{ / }
								<select title="Search Type" name="search_fld">
									<option value="all"{ ? _GET.search_fld == 'all' } selected="selected"{ / }>All</option>
									<option value="subject"{ ? _GET.search_fld == 'subject' } selected="selected"{ / }>Subject</option>
									<option value="contents"{ ? _GET.search_fld == 'contents' } selected="selected"{ / }>Contents</option>
									<option value="writer_name"{ ? _GET.search_fld == 'writer_name' } selected="selected"{ / }>Name</option>
								</select>
								<input type="text" name="search" value="{ =htmlspecialchars( _GET.search ) }" title="Please enter your search string." />
								<a href="#" class="sbtn act_board_search">Search</a>
								</form>
							</div>
						</div>
					</div>
					...
					<div class="bbs">
							...
							<table cellpadding="0" cellspacing="0" summary="게시판입니다.">
								...
								<thead>
									<tr>
										...
										<th scope="col" class="mb_hd">Hits <button class="btn_ord up" data-fld="hits"></button><button class="btn_ord down" data-fld="hits"></button></th>
									</tr>
								</thead>
								<tbody>
								{ @ LIST }
						...
						<div class="btnBbs ar">
							<div class="right">
								<a href="{ BASE_URL }/{ MENU_NO }/{ CONTENTS['idno'] }/write?{ QS }"{ ? CAN_WRITE != 'Y' } class="no_auth{ CONF['auth_write'] }"{ / }>Write</a>
							</div>
						</div>
					</div>
