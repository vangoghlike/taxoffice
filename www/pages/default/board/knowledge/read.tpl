...
{ CONTENTS['head_contents'] }
							...
							<div class="left">
								<div class="box">
									...
									{ ? DATA['kl_manager_name'] == '' }
										...
										하유정 세무사 ( <a href="mailto:{ DATA['kl_email_manager'] }">{ DATA['kl_email_manager'] }</a> )
										...
										{ / }
									{ : }
										{ DATA['kl_manager_name'] } ( <a href="mailto:{ DATA['kl_email_manager'] }">{ DATA['kl_email_manager'] }</a> )
									{ / }
									</div>
								</div>
							</div>
					...
					<div class="btnBbs bbNone ">
						...
						<div class="right">
							{ ? USERINFO['user_id'] == '' }
								{ ? DATA['kl_status'] == 'N' }
							<a href="{ BASE_URL }/{ MENU_NO }/{ CONTENTS['idno'] }/write?{ QS }&idno={ DATA['idno'] }&udt=nrply" class="{ ? CONF['auth_write'] == 'N' }board_secretY{ / }" data-idno="{ DATA['idno'] }">수정</a>
								{ : }
							...
							<a href="{ BASE_URL }/{ MENU_NO }/{ CONTENTS['idno'] }/write?{ QS }&idno={ DATA['idno'] }&udt=nrply" class="{ ? CONF['auth_write'] == 'N' }{ / }" data-idno="{ DATA['idno'] }">수정</a>
									{ : }
							<a class="gray_btn">답변완료 질문</a>
							...
							<a href="{ BASE_URL }/{ MENU_NO }/{ CONTENTS['idno'] }/?{ QS }" class="act_delete" data-idno="{ DATA['idno'] }">삭제</a>
								{ : }
									{ ? DATA['kl_status'] == 'N' }
							<a href="{ BASE_URL }/{ MENU_NO }/{ CONTENTS['idno'] }/write?{ QS }&idno={ DATA['idno'] }&udt=nrply" class="{ ? DATA['reg_user_id'] != USERINFO['user_id'] }board_secretY{ / }" data-idno="{ DATA['idno'] }">수정</a>
							<a href="{ BASE_URL }/{ MENU_NO }/{ CONTENTS['idno'] }/?{ QS }" class="act_delete { ? DATA['reg_user_id'] != USERINFO['user_id'] }board_secretY{ / }" data-idno="{ DATA['idno'] }">삭제</a>
									{ : }
							<a class="gray_btn">답변완료 질문</a>
							<a href="{ BASE_URL }/{ MENU_NO }/{ CONTENTS['idno'] }/?{ QS }" class="act_delete { ? CONF['auth_write'] == 'N' }board_secretY{ / }" data-idno="{ DATA['idno'] }">삭제</a>
									{ / }
					...
					<table summary="윗글/아랫글" class="basic_tb">
						...
						<tbody>
							<tr class="prev">
								<th scope="row"><span class="up_bul" style="font-size:0.75rem;">이전글</span></th>
							...
							<tr>
								<th scope="row"><span class="down_bul" style="font-size:0.75rem;">다음글</span></th>
