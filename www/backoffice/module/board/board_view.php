	...
	switch($bMode){
		...
		default:
			...
			if($_SESSION[$_SITE["DOMAIN"]]["ADMIN"]["ID"] || $_SESSION[$_SITE["DOMAIN"]]["MEMBER"]["LEVEL"] >= $arrBoardInfo["list"][0]["listlevel"]){
				 if($arrBoardInfo["list"][0]['skin']=="gallery"){
				...
				}else if($arrBoardInfo["list"][0]["boardid"] == "community"){
					...
					$arrBoardList = getBoardListBaseNFile($arrBoardInfo["list"][0]["boardid"], $_GET["category"], $_GET['sw'], $_GET['sk'], $arrBoardInfo["list"][0]["scale"], $_GET['offset']);
					// $arrBoardList = getBoardListBaseNFile($arrBoardInfo["list"][0]["boardid"], postNullCheck('category'), postNullCheck('sw'), postNullCheck('sk'), $arrBoardInfo["list"][0]["scale"], postNullCheck('offset'));
				}
