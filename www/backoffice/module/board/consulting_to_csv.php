...
header( "Content-Disposition: attachment; filename=".iconv("UTF-8","UTF-8",$_SITE['NAME'])."_상담신청내역_".date(m)."월".date(d)."일".date(h)."시".date(i)."분.csv" );
...
for ( $i=0 ; $i<$arrBoardList["list"]["total"] ; $i++ ) {
	if($arrBoardList["list"][$i]['r_user']==""){
		$r_user="접수";
	}else if($arrBoardList["list"][$i]['r_user']=="E"){
		$r_user="처리완료";
	}
	...
	echo $i+1 . ",";
	echo strip_tags(str_replace(",",".", stripslashes(iconv("UTF-8","UTF-8",$r_user))))  . ",";
	echo strip_tags(str_replace(",",".", stripslashes(iconv("UTF-8","UTF-8",$arrBoardList["list"][$i]['subject']))))  . ",";
	echo strip_tags(str_replace(",",".", stripslashes(iconv("UTF-8","UTF-8",$arrBoardList["list"][$i]['pass']))))  . ",";
