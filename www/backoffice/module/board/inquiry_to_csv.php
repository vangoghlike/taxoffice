...
header( "Content-Disposition: attachment; filename=".iconv("UTF-8","UTF-8",$_SITE['NAME'])."_기술이전문의_".date(m)."월".date(d)."일".date(h)."시".date(i)."분.csv" );
...
for ( $i=0 ; $i<$arrBoardList["list"]["total"] ; $i++ ) {
	if($arrBoardList["list"][$i]['etc_4']==""){
		$etc_4="접수";
	}else if($arrBoardList["list"][$i]['etc_4']=="E"){
		$etc_4="처리완료";
	}
	...
	echo $i+1 . ",";
	echo strip_tags(str_replace(",",".", stripslashes(iconv("UTF-8","UTF-8",$etc_4))))  . ",";
	echo strip_tags(str_replace(",",".", stripslashes(iconv("UTF-8","UTF-8",$arrBoardList["list"][$i]['category']))))  . ",";
	...
	echo strip_tags(str_replace(",",".", stripslashes(iconv("UTF-8","UTF-8",$arrBoardList["list"][$i]['etc_3']))))  . ",";
