...
if($_REQUEST["idx"]) {
	$arrInfo = getGiftcardInfo(mysql_escape_string($_REQUEST["idx"]));
	$btn_name = "상품권 수정";
	$history_name = "상품권 수정";
	$mode = "modify";

	$arrCUList = getGiftcardUserListAdmin(mysql_escape_string($_REQUEST["idx"]), $scale, mysql_escape_string($_REQUEST[offset]));
} else {
	$btn_name = "상품권 발행";
	$history_name = "상품권 등록";
	$mode = "add";
...
function checkForm(f){
	if(f.giftcard_name.value.length < 1){
		alert("상품권명을 입력하세요.");
		f.giftcard_name.focus();
		return false;
	}
	if(f.giftcard_dis.value.length < 1){
		alert("할인금액(율)을 입력하세요.");
		f.giftcard_dis.focus();
		return false;
	}
	<? if($mode == "add") {?>
	if(f.giftcard_qty.value.length < 1){
		alert("발급수량을 입력하세요.");
		f.giftcard_qty.focus();
		return false;
	}
	<?}?>
}
...
<script language="javascript">
function giftcardUserDel(id){
	var cfm;
	cfm =false;
	cfm = confirm("발급된 상품권을 삭제 하시겠습니까?");
	if(cfm==true){
...
function addMember(no) {
	var win = window.open("/backoffice/module/member/search_member.php?idx="+no+"&gb=G","회원찾기","width=1000,height=700,scrollbars=yes");
	win.focus();
}
</script>
<script language="javascript">
...
<!-- S 개인정보입력 -->
<form name="frmPoint" method="post" action="/backoffice/module/giftcard/giftcard_evn.php" onsubmit="return checkForm(this);">
...
<table class="admin-table-type1">
  ...
  <tbody>
	<tr>
		<th>상품권명</th>
		<td class="space-left"><input type="text" name="giftcard_name" style="width:200px" class="input" value="<?=stripslashes($arrInfo["list"][0][giftcard_name])?>"></td>
	</tr>
	<tr>
		<th>상품권설명</th>
		<td class="space-left"><textarea name="giftcard_content" class="input" style="width:500px;height:40px"><?=stripslashes($arrInfo["list"][0][giftcard_content])?></textarea></td>
	</tr>
	<tr>
		<th>유효기간</th>
		<td class="space-left"><input type="text" name="giftcard_sdate" style="width:100px" class="input datePicker" value="<?=stripslashes($arrInfo["list"][0][giftcard_sdate])?>"> ~ 
			<input type="text" name="giftcard_edate" style="width:100px" value="<?=stripslashes($arrInfo["list"][0][giftcard_edate])?>" class="input datePicker"></td>
	</tr>
	<tr>
		<th>상품권적용 상한가</th>
	...
	<tr>
		<th>상품권금액(할인율)</th>
		<td class="space-left"><input type="text" name="giftcard_dis" style="width:100px" class="input" maxlength="20" value="<?=stripslashes($arrInfo["list"][0][giftcard_dis])?>">
		<select name="giftcard_unit">
		<option value="P"<?=$arrInfo["list"][0][giftcard_unit]=="P"?" selected":""?>>%</option>
		<option value="F"<?=$arrInfo["list"][0][giftcard_unit]=="F"?" selected":""?>>고정금액</option>
		</select>
		</td>
	</tr>
	<tr>
		...
		<td class="space-left">
		<? if($mode == "modify") {?>
		<?=number_format($arrInfo["list"][0][giftcard_qty])?>개, 추가발행: <input type="text" name="giftcard_qty" style="width:40px" class="input" maxlength="3" value=""> 숫자만 입력하세요. (수량만큼 상품권이 생성됩니다.)
		<?}else{?>
		<input type="text" name="giftcard_qty" style="width:40px" maxlength="3" class="input" value="<?=number_format($arrInfo["list"][0][giftcard_qty])?>"> 숫자만 입력하세요. (수량만큼 상품권이 생성됩니다.)
		<?}?></td>
	</tr>
	<tr>
		<td colspan="2" align="center"><input type="button" name="btn" value="목록보기" style="width:100px;height:40px;font-weight:bold" onclick="document.location.href='giftcard.php'"> <input type="submit" value="<?=$btn_name?>" style="width:100px;height:40px;color:blue;font-weight:bold"></td>
	</tr>	
	</form>
</tbody>
</table>
...
<h3 class="admin-title-middle">발행된 상품권현황</h3>
<div class="clfix mgb5">
    <div class="fl" style="padding-top:4px;">&nbsp;<strong>전체 : <?=number_format($arrCUList['total'])?> 명</strong></div>
    <div class="fr"><span class="btn_pack medium icon"><span class="download"></span><a href="giftcard_to_csv.php?idx=<?=$_GET[idx]?>">CSV 파일로 다운로드</a></span> 
</div>
<table border="0" cellpadding="0" cellspacing="1" width="100%">
<tr height="25" align="center" bgcolor="#646464">
  <td width="5%"><font color="#ffffff">No.</font></td>
  <td width="20%"><font color="#ffffff">상품권번호</font></td>
...
<tr height="25" align="center">
  ...
  <td><?=$arrCUList['list'][$i]['giftcard_no']?></td>
  <td><?=$arrCUList['list'][$i]['user_id']?></td>
  <td><?=$arrCUList['list'][$i]['user_name']?></td>
  <td><?=$arrCUList['list'][$i]['giftcard_use']?></td>
  <td><?=substr($arrCUList['list'][$i]['udate'],0,10)?></td>
  <td><?=substr($arrCUList['list'][$i]['wdate'],0,10)?></td>
  <td align="center">
  <? if($arrCUList['list'][$i]['user_id']) {?>
  <!-- <a href="giftcard_evn.php?evnMode=sendGiftcard&user_id=<?=$arrCUList['list'][$i]['user_id']?>&user_name=<?=$arrCUList['list'][$i]['user_name']?>&idx=<?=$arrCUList['list'][$i]['idx']?>"><font color='blue'>메일발송</font></a> |  -->
  <?}else{?>
  <a href="javascript:addMember(<?=$arrCUList['list'][$i]['idx']?>)">회원등록</a> | 
  <?}?><a href="javascript:giftcardUserDel('<?=$arrCUList['list'][$i]['idx']?>')">삭제</a></td>
</tr>
<tr>
...
<tr height="100" align="center">
  <td width="100%" colspan="8" >등록된 상품권이 없습니다.</td>
</tr>
<tr>
  <td colspan="10" height="1" bgcolor="646464"></td>
</tr>
