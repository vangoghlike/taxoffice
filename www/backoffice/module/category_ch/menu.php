...
<div class="admin-snb">
	...
	<ul class="snb-menu">
	<li><a href="/backoffice/module/category_ch/category.php">메뉴 관리</a></li>
	</ul>
	<?
	...
	?>
		...
		<?if($arrSubMenuList["total"] > 0){ // 하위 메뉴가 있을 경우?> 
			...
			<a style="color:white;" href="/backoffice/module/category_ch/category_info.php?cat_no=<?=$arrMenuList["list"][$i]["cat_no"]?>">[폴더수정]</a></h2>
		<?}else{ // 하위 메뉴가 없을 경우?>
			<a href="/backoffice/module/category_ch/category_setting.php?cat_no=<?=$arrMenuList["list"][$i]["cat_no"]?>"><h2 <?if($arrMenuList["list"][$i]["cat_no"] == $_GET["cat_no"]){?>class="bold"<?}?>><?=text_cut($arrMenuList["list"][$i]["cat_name"],15)?></h2></a>
		<?}?>
		</div>
		<ul class="snb-menu">
		<?
		...
		?>
			<li><a id="sub_<?=$arrSubMenuList["list"][$j]["cat_no"]?>" href="/backoffice/module/category_ch/category_setting.php?cat_no=<?=$arrSubMenuList["list"][$j]["cat_no"]?>"><?=$arrSubMenuList["list"][$j]["cat_name"]?></a></li>
		<?
				}else{
		?>
			<li class="tab" style="cursor:pointer;"><a <?if($arrSubMenuList["list"][$j]["cat_no"] == $_GET["cat_no"]){?>class="bold"<?}?> id="folder_<?=$arrSubMenuList["list"][$j]["cat_no"]?>" style="display:inline-block;"><?=$arrSubMenuList["list"][$j]["cat_name"]?></a><span style="display:inline-block;z-index:10;"><a  href="/backoffice/module/category_ch/category_info.php?cat_no=<?=$arrSubMenuList["list"][$j]["cat_no"]?>">[폴더수정]</a></span>
				<ul class="tab_ul">
					<?
					if($arrSub_3MenuList["total"] > 0){
						for($k=0;$k<$arrSub_3MenuList["total"];$k++){
							...
							?>
								<li><a id="sub_<?=$arrSub_3MenuList["list"][$k]["cat_no"]?>" href="/backoffice/module/category_ch/category_setting.php?cat_no=<?=$arrSub_3MenuList["list"][$k]["cat_no"]?>"><?=$arrSub_3MenuList["list"][$k]["cat_name"]?></a></li>
							<?
							}else{
?>
								<li class="tab" style="cursor:pointer;"><a <?if($arrSub_3MenuList["list"][$k]["cat_no"] == $_GET["cat_no"]){?>class="bold"<?}?> id="folder_<?=$arrSub_3MenuList["list"][$k]["cat_no"]?>" style="display:inline-block;"><?=$arrSub_3MenuList["list"][$k]["cat_name"]?></a><span style="display:inline-block;z-index:10;"><a href="/backoffice/module/category_ch/category_info.php?cat_no=<?=$arrSub_3MenuList["list"][$k]["cat_no"]?>">[폴더수정]</a></span>
									<ul class="tab_ul">
<?
								if($arrSub_4MenuList["total"] > 0){
									for($l=0;$l<$arrSub_4MenuList["total"];$l++){
?>
										<li><a id="sub_<?=$arrSub_4MenuList["list"][$l]["cat_no"]?>" href="/backoffice/module/category_ch/category_setting.php?cat_no=<?=$arrSub_4MenuList["list"][$l]["cat_no"]?>"><?=$arrSub_4MenuList["list"][$l]["cat_name"]?></a></li>
<?
									}
								}
?>
