...
function getCategoryFree($que_where){
	...
	$_add_query = '';
	if ( strpos($que_where,'hanpage') !== false ) {
		$tbl = $GLOBALS["_conf_tbl"]["category"].'_hanpage';
	} else if ( strpos($que_where,'en_latest') !== false ) {
		$tbl = $GLOBALS["_conf_tbl"]["category"].'_eng';
	}else {
		$tbl = $GLOBALS["_conf_tbl"]["category"];
		$_add_query = " AND `cat_is_show` = 'Y' ";
	}

	$sql  = "SELECT * ";
	$sql .= "FROM ".$tbl." ";
	$sql .= "WHERE 1=1 $_add_query $que_where ";
...
function getCategoryList($cat_no,$cat_gubun="", $viewgb=""){
	...
	if($cat_no==""){
		$sql  = "SELECT * ";
		$sql .= "FROM ".$tbl." ";
		$sql .= "WHERE cat_depth='0' $que_where order by cat_no ASC ";

		$rs = mysqli_query($GLOBALS['dblink'], $sql);
		$total_rs = mysqli_num_rows($rs);

	}else{
		//cat_no(ī�װ����ȣ) ���� ī�װ�� �ڵ�� ī�װ�����̸� ã��
