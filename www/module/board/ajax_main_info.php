...
if (strpos($_SERVER['HTTP_HOST'], 'taxoffice.co.kr') !== false) {
    $selim_main_url = '';
} else {
    $selim_main_url = 'http://www.taxoffice.co.kr';
}
...
<?
    ...
    $_location = $selim_main_url."/sub/?cat_no=21";
    $location = $selim_main_url."/sub/?boardid=hanpage&cat_no=21&sk=";
    $arrList = getBoardListBaseNFile($_REQUEST["boardid"], "", "", "", 4, 0);
...
?>
            ...
            <h3>고객님께
                <span class="basic_span">
                    <span class="point_txt">
                    꼭
                    </span>
                    <img class="point_img" src="/pub/images/main/vcheck.png" alt="끼움 이미지"/>
                </span>
                필요한
                <strong class="highlight">세무정보</strong>를 <span class="br mo"></span>가장 쉽고 유용한 상태로 <span class="br mo"></span>제공합니다.</h3>
            <ul class="info_tab">
                <li <?if($_REQUEST["boardid"] == "total"){?>class="on"<?}?> onclick="getBoardAjax('total')">최신글</li>
                <li <?if($_REQUEST["boardid"] == "hanpage"){?>class="on"<?}?> onclick="getBoardAjax('hanpage')">한페이지</li>

                <li <?if($_REQUEST["boardid"] == "topic_business_all"){?>class="on"<?}?> onclick="getBoardAjax('topic_business_all')">사업관련세금</li>
                <li <?if($_REQUEST["boardid"] == "topic_property_all"){?>class="on"<?}?> onclick="getBoardAjax('topic_property_all')">재산관련세금</li>

                <li <?if($_REQUEST["boardid"] == "joseNews"){?>class="on"<?}?> onclick="getBoardAjax('joseNews')">조세뉴스</li>
            </ul>
