...
<div id="admin-content">
	...
	<table class="listTable">
		...
		?>
		<tr>
			<td colspan="8" align="center">No posts were found.</td>
		</tr>
		<?
		}
		?>		
...
<script>
    ...
    <p style="padding:2.0rem; text-align: center; font-size:1.25rem;">
        The tax information question and answer board is being prepared.
    </p>

<div class="tabType01 kl-type">
    <div class="kl-cate-wr">
        <ul>
            <li class="on"><a href="./index.php?cat_no=<?=$_GET['cat_no']?>&boardid=hanpage&mode=list&sk=&sw=&offset=&category=">Tax Info at a Glance</a></li>
            <li class="qna_li">
                <a href="<?="?cat_no=".$_GET['cat_no']."&boardid=".$arrBoardInfo["list"][0]["boardid"]?>&mode=write">Client Inquiry Center</a>
            </li>
        </ul>
    </div>
    <?php if ( $_REQUEST['category'] != 'Asked' ) { ?>
        <ul class="hanpage_cate_wr <?=$_REQUEST['category'] != ''?'off':''?>">
            <li <?php if($_REQUEST['category'] == 'TransferIncomeTax'){echo 'class="on"';} ?>><a href="./index.php?cat_no=<?=$_REQUEST['cat_no']?>&boardid=eng_hanpage&mode=list&sk=&sw=&offset=&category=TransferIncomeTax">Transfer Income Tax</a></li>
            <li <?php if($_REQUEST['category'] == 'InheritanceTax'){echo 'class="on"';} ?>><a href="./index.php?cat_no=<?=$_REQUEST['cat_no']?>&boardid=eng_hanpage&mode=list&sk=&sw=&offset=&category=InheritanceTax">Inheritance Tax</a></li>
            <li <?php if($_REQUEST['category'] == 'GiftTax'){echo 'class="on"';} ?>><a href="./index.php?cat_no=<?=$_REQUEST['cat_no']?>&boardid=eng_hanpage&mode=list&sk=&sw=&offset=&category=GiftTax">Gift Tax</a></li>
            <li <?php if($_REQUEST['category'] == 'LocalTax'){echo 'class="on"';} ?>><a href="./index.php?cat_no=<?=$_REQUEST['cat_no']?>&boardid=eng_hanpage&mode=list&sk=&sw=&offset=&category=LocalTax">Local Tax</a></li>
            <li <?php if($_REQUEST['category'] == 'ComprehensiveRealEstateHoldingTax'){echo 'class="on"';} ?>><a href="./index.php?cat_no=<?=$_REQUEST['cat_no']?>&boardid=eng_hanpage&mode=list&sk=&sw=&offset=&category=ComprehensiveRealEstateHoldingTax"><!--Comprehensive Real -->Estate Holding Tax</a></li>
            <li <?php if($_REQUEST['category'] == 'IncomeTax'){echo 'class="on"';} ?>><a href="./index.php?cat_no=<?=$_REQUEST['cat_no']?>&boardid=eng_hanpage&mode=list&sk=&sw=&offset=&category=IncomeTax">Income Tax</a></li>
            <li <?php if($_REQUEST['category'] == 'CorporateTax'){echo 'class="on"';} ?>><a href="./index.php?cat_no=<?=$_REQUEST['cat_no']?>&boardid=eng_hanpage&mode=list&sk=&sw=&offset=&category=CorporateTax">Corporate Tax</a></li>
            <li <?php if($_REQUEST['category'] == 'VAT_ExImport'){echo 'class="on"';} ?>><a href="./index.php?cat_no=<?=$_REQUEST['cat_no']?>&boardid=eng_hanpage&mode=list&sk=&sw=&offset=&category=VAT_ExImport">VAT & Ex/Import</a></li>
            <li <?php if($_REQUEST['category'] == 'Payroll_4SocialInsurance'){echo 'class="on"';} ?>><a href="./index.php?cat_no=<?=$_REQUEST['cat_no']?>&boardid=eng_hanpage&mode=list&sk=&sw=&offset=&category=Payroll_4SocialInsurance">Payroll & 4 Social Insurance</a></li>
            <li <?php if($_REQUEST['category'] == 'Bookkeeping'){echo 'class="on"';} ?>><a href="./index.php?cat_no=<?=$_REQUEST['cat_no']?>&boardid=eng_hanpage&mode=list&sk=&sw=&offset=&category=Bookkeeping">Bookkeeping (accounting)</a></li>
            <li <?php if($_REQUEST['category'] == 'FDI'){echo 'class="on"';} ?>><a href="./index.php?cat_no=<?=$_REQUEST['cat_no']?>&boardid=eng_hanpage&mode=list&sk=&sw=&offset=&category=FDI">FDI</a></li>
            <li <?php if($_REQUEST['category'] == 'Stocks'){echo 'class="on"';} ?>><a href="./index.php?cat_no=<?=$_REQUEST['cat_no']?>&boardid=eng_hanpage&mode=list&sk=&sw=&offset=&category=Stocks">Stocks</a></li>
            <li <?php if($_REQUEST['category'] == 'RealEstateLease'){echo 'class="on"';} ?>><a href="./index.php?cat_no=<?=$_REQUEST['cat_no']?>&boardid=eng_hanpage&mode=list&sk=&sw=&offset=&category=RealEstateLease">(Housing) Real Estate Lease</a></li>
            <li <?php if($_REQUEST['category'] == 'GeneralAffairs'){echo 'class="on"';} ?>><a href="./index.php?cat_no=<?=$_REQUEST['cat_no']?>&boardid=eng_hanpage&mode=list&sk=&sw=&offset=&category=GeneralAffairs">General Affairs</a></li>
        </ul>
        <?php if ($_REQUEST['category'] != '') { ?>
            <a class="hanpage_cate_onoff">
                <strong>Expand All Categories</strong>
            </a>
        <?php } ?>
    <?php } ?>
    <?php if ( $_REQUEST['category'] == 'Asked' ) { ?>
        <h3 class="tac">Q&A waiting list</h3>
    <?php } ?>
</div>
...
<div class="side">
	...
	<div class="right">
		<div class="searchArea">
			<form name="form1" method="get" action="<?=$_SERVER[PHP_SELF]?>"> 
				<?if($_REQUEST["ord"] != ""){?>
				<a class="btn_box green btn_ord init" onclick="ordReset()">In basic order</a>
				<?}?>
				<input type="hidden" name="boardid" value="<?=$arrBoardInfo["list"][0]["boardid"]?>">
				<input type="hidden" name="category" value="<?=$_GET["category"]?>">
				<input type="hidden" name="cat_no" value="<?=$_GET["cat_no"]?>">
				<select name="sw" title="검색 분류선택">
					<option value='a'<?=$_GET[sw]=="a"?" selected='selected'":""?>>All</option>
					<option value='s'<?=$_GET[sw]=="s"?" selected='selected'":""?>>Subject</option>
					<option value='c'<?=$_GET[sw]=="c"?" selected='selected'":""?>>Contents</option>
					<option value='n'<?=$_GET[sw]=="n"?" selected='selected'":""?>>Name</option>
				</select>
				<input type="text" title="검색 입력란" name="sk" value="<?=$_GET[sk]?>" />
				<a href="javascript:void(0);" class="sbtn act_board_search" onclick="document.form1.submit()">Search</a>
			</form>
		</div>
	</div>
</div>
<div class="bbs">
		...
		<table class="listTable" cellpadding="0" cellspacing="0" summary="게시판입니다.">
			...
			<thead>
				<tr>
					...
					<th scope="col">Subject <button class="btn_ord up" onclick="ordSort('subject')"></button><button class="btn_ord down" onclick="ordSort('subject desc')"></button></th>
					<th scope="col" class="mb_hd">File</th>
					<th scope="col">Counselor <button class="btn_ord up" onclick="ordSort('name')"></button><button class="btn_ord down" onclick="ordSort('name desc')"></button></th>
					<th scope="col">Date <button class="btn_ord up" onclick="ordSort('wdate')"></button><button class="btn_ord down" onclick="ordSort('wdate desc')"></button></th>
					<th scope="col" class="mb_hd">Hits <button class="btn_ord up" onclick="ordSort('hit')"></button><button class="btn_ord down" onclick="ordSort('hit desc')"></button></th>
				</tr>
			</thead>
			<tbody>
			<?
			...
			?>
				<tr <?=$TrClass?>>
                    ...
                    <td class="td_name"><?=$arrBoardList["list"][$i]['category']?></td>
			...
			?>
				<tr class="">
					<td class="td_num" colspan="7">There are no registered post.</td>
				</tr>
			<?
			}
			?>
	...
	<!-- //blist -->
	<?if($_SESSION[$GLOBALS["_SITE"]["DOMAIN"]]["MEMBER"]["LEVEL"] == "99"){ // 관리자 권한이 있다면?>
	<div class="btnBbs ar">
		<div class="right">
			<a href="<?=$_SERVER[PHP_SELF]?>?cat_no=<?=$_GET["cat_no"]?>&boardid=<?=$arrBoardInfo["list"][0]["boardid"]?>&mode=write">write</a>
		</div>
	</div>
	<?}?>
</div>
