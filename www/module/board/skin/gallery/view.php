...
<div class="viewWrap">
	...
	<div class="viewContent">
        <?php if ( $arrBoardArticle["total_files"] ) { ?>
            <?
            for($i=0;$i<$arrBoardArticle["total_files"];$i++){
                if(substr($arrBoardArticle["files"][$i][re_name],0,2) != "l_") {
                    $imgsrc[$i] = "/uploaded/board/".$arrBoardInfo["list"][0]["boardid"]."/".$arrBoardArticle["files"][$i][re_name];
                    ?>
                    <div class="img_content"><img src="<?=$imgsrc[$i]?>"/></div>
                <?}}?>
        <? } ?>
...
<div class="btnBbs bbNone ">
    ...
    <div class="right">
        <a href="<?=$_SERVER[PHP_SELF]?>?cat_no=<?=$_GET["cat_no"]?>&boardid=<?=$arrBoardInfo["list"][0]["boardid"]?>&mode=modify&idx=<?=$arrBoardArticle["list"][0][idx]?>&category=<?=$_GET[category]?>" class="btn_box act_upt">수정</a>
    </div>
</div>
