...
<div class="side">
	<div class="left">
		<div class="countTotal">Total : <?=$arrBoardList["total"]?></div>
	</div>
	<div class="right">
	...
	<ul>
		...
		if($arrBoardList["list"]["total"] > 0){
			for($i=0; $i < $arrBoardList["list"]["total"]; $i++){
				...
				$fileImg = "";
				$imgsrc[$i] = "";
				if($arrBoardList["list"][$i]["total_files"] > 0){
					for($j=0;$j<$arrBoardList["list"][$i]["total_files"];$j++){
						if(substr($arrBoardList["list"][$i]["files"][$j]["re_name"],0,2) == "l_"){
							$imgsrc[$i] = "/uploaded/board/".$arrBoardInfo["list"][0]["boardid"]."/".$arrBoardList["list"][$i]["files"][$j]["re_name"];
						}
					}
				}
				if($imgsrc[$i] == ""){
		...
		?>
		<li>
			<a href="<?=$_SERVER[PHP_SELF]?>?cat_no=<?=$_GET["cat_no"]?>&boardid=<?=$arrBoardInfo["list"][0]["boardid"]?>&mode=view&idx=<?=$arrBoardList["list"][$i][idx]?>&sk=<?=$_GET[sk]?>&sw=<?=$_GET[sw]?>&offset=<?=$_GET[offset]?>&category=<?=$_GET[category]?>" class="">
				<div class="img">
					<img src="<?=$imgsrc[$i]?>" alt="썸네일 이미지" width="240" height="180">
				</div>
				<div class="txt"><?=$arrBoardList["list"][$i]["subject"]?></div>
			</a>
		</li>
		<?
			}
...
<div class="btnBbs ar">
	<div class="right">
		<a href="<?=$_SERVER[PHP_SELF]?>?cat_no=<?=$_GET["cat_no"]?>&boardid=<?=$arrBoardInfo["list"][0]["boardid"]?>&mode=write">글쓰기</a>
	</div>
</div>
<?}?>
