...
<?
	$arrETC = array("양도소득세","지방세","종부세","증여세","상속세","법인세","부가세");	 // 수정시 업무에 관련 option 변경 됩니다.
?>
<div id="admin-content">
	...
	<table class="writeTable">
		...
		<tbody>
		<tr>
			<th class="nec">세무사명</th>
			<td colspan="3"><input type="text" class="txt req" style="width:150px" name="name" id="name" value="<?=$arrBoardArticle["list"][0][name]?>" maxlength="10" title="세무사명을 입력해주세요."  /></td>
		</tr>
		<!-- <tr>
			<th class="nec">세무사 문구</th>
			<td colspan="3">
				<input type="text" class="txt req" style="width:70%" name="subject" id="subject" value="<?=stripslashes($arrBoardArticle["list"][0][subject])?>" maxlength="20" title="제목을 입력해주세요." />
				<div class="subject_notice"></div>
			</td>
		</tr> -->
		<tr>
			<th class="nec">업무 1</th>
			<td colspan="3">
				<input type="text" class="txt req" style="width:70%" name="etc_1" id="etc_1" value="<?=stripslashes($arrBoardArticle["list"][0][etc_1])?>" maxlength="7" />
			</td>
		</tr>
		<tr>
			<th class="nec">업무 2</th>
			<td colspan="3">
				<input type="text" class="txt req" style="width:70%" name="etc_2" id="etc_2" value="<?=stripslashes($arrBoardArticle["list"][0][etc_2])?>" maxlength="7" />
			</td>
		</tr>
		<tr>
			<th class="nec">업무 3</th>
			<td colspan="3">
				<input type="text" class="txt req" style="width:70%" name="etc_4" id="etc_4" value="<?=stripslashes($arrBoardArticle["list"][0][etc_4])?>" maxlength="7" />
			</td>
		</tr>
		<tr>
			<th class="nec">업무 4</th>
			<td colspan="3">
				<input type="text" class="txt req" style="width:70%" name="etc_5" id="etc_5" value="<?=stripslashes($arrBoardArticle["list"][0][etc_5])?>" maxlength="7" />
			</td>
		</tr>
		<tr>
			<th class="nec">순번</th>
			<td colspan="3">
				<input type="number" class="txt req" style="width:10%" name="etc_3" id="etc_3" value="<?if($_REQUEST[mode]=="modify"){?><?=$arrBoardArticle["list"][0][etc_3]?><?}else{?>0<?}?>" min="0" />
				※ 숫자가 높을 수록 먼저 표시 됩니다.
			</td>
		</tr>
		<?
...
<div class="notice container">
	...
	<form name="form1" method="post" action="/module/board/board_evn.php" ENCTYPE="multipart/form-data"  class="adminForm">
		...
		<input type="hidden" name="usehtml" value="Y">
		<?}?>
		<table class="writeTable text-left">
			<caption>글쓰기</caption>
			<colgroup>
			...
			<tbody>
				...
				<tr>
					...
					<th>일시</th>
					<td>
					  <input type="text" name="schedule_date" id="schedule_date" value="<?if($_REQUEST[mode]=="modify"):?><?=stripslashes($arrBoardArticle["list"][0][schedule_date])?><?else:?><?=date("Y-m-d", time())?><?endif;?>" class="datePicker wid">
					</td>
				</tr>
				<tr>
					<th>제목</th>
					<td>
						<input class="wid" type="text" name="subject" id="subject" value="<?=stripslashes($arrBoardArticle["list"][0][subject])?>" placeholder="제목을 입력하세요.">						
					</td>
					<th>공지</th>
					<td>
						<select name="is_notice">						
							<option value=""<?if($arrBoardArticle["list"][0][no]!="0"){echo " selected";}?>>일반</option>
							<option value="Y"<?if($arrBoardArticle["list"][0][no]=="0"){echo " selected";}?>>공지</option>
						</select>
					</td>
				</tr>
				<tr>
						...
						<table id="files_table" border="0" cellpadding="3" cellspacing="1" width="100%" style="padding:1%">
							...
							<tr height="25">
								<td align='left' width='100%'>
									<input name="upfiles[]" type="file" class="type01"  style="width: 400px;">
									<a href="javascript:appendfile();"><img src="/images/btnPlus3.png" alt="파일추가"></a>
									<a href="javascript:remove();"><img src="/images/btnMin3.png" alt="파일삭제"></a>
								</td>
							</tr>
							<tr>
								<td align='left' width='100%' id="filetd">123
								<input name="upfiles[]" type="file" class="type01"  style="width: 400px;">
								</td>
							</tr>
							</tbody>
		...
		<div class="wirteBtn text-center clear">
			<button class="redBtn" type="button" onclick="frmCheck(document.form1);">글쓰기</button>
			<a href="<?=$_SERVER[PHP_SELF]?>?boardid=<?=$arrBoardInfo["list"][0]["boardid"]?>&mode=list&category=<?=$_GET[category]?>" class="blackBtn text-center">목록</a>
		</div>
	</form>
	<!--Write   ST-->
</div>
