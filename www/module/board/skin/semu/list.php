...
<div id="admin-content">
	...
	<table class="listTable">
		...
		<tr>
			...
			<td><?=$categoryTitle?></td>
			<td><img src="<?=$imgsrc[$i]?>" style="max-width:100%;max-height:80px;"></td>
			<td><?=$arrBoardList["list"][$i][name]?></td>
			<td class="al"><?=text_cut(stripslashes($arrBoardList["list"][$i][subject]),$arrBoardInfo["list"][0][subjectcut])?></td>
			<td><?=$arrBoardList["list"][$i][etc_1]?></td>
		...
		<table cellpadding="0" cellspacing="0" summary="게시판입니다.">
			...
			?>
				<tr class="">
					...
					<td class="subject"><a href="<?=$_SERVER[PHP_SELF]?>?cat_no=<?=$_GET["cat_no"]?>&boardid=<?=$arrBoardInfo["list"][0]["boardid"]?>&mode=view&idx=<?=$arrBoardList["list"][$i][idx]?>&sk=<?=$_GET[sk]?>&sw=<?=$_GET[sw]?>&offset=<?=$_GET[offset]?>&category=<?=$_GET[category]?>" class="" ><?=text_cut($arrBoardList["list"][$i][subject],50)?></a> <?=$newImage?></td>
