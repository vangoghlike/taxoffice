	...
	<table class="writeTable">
		...
		<tr>
			...
			<td colspan="3">
				...
				<?
					$arrTCate = explode(",",$arrBoardInfo["list"][0]["category"]);
					for($i=0;$i<count($arrTCate);$i++){
				?>
					<option value="<?=$arrTCate[$i]?>" <?if($arrTCate[$i] == $arrBoardArticle["list"][0]["category"]){?>selected<?}?>><?=$arrTCate[$i]?></option>
				<?
					}	
				?>
				</select>
		...
		<tr>
			...
			<td colspan="3">
				...
				<table id="files_list" border="0" cellpadding="3" cellspacing="1" width="100%" style="padding:1%">
					...
					?>
						<tr> 
							<td style="text-align:left;"><input type="checkbox" name="filedel[]" value="<?=$arrBoardArticle["files"][$i][idx]?>" id="filedel_<?=$i?>"><label for="filedel_<?=$i?>">삭제</label> : <a href="/uploaded/board/<?=$arrBoardInfo["list"][0]["boardid"]?>/<?=$arrBoardArticle["files"][$i][re_name]?>" download="<?=$arrBoardArticle["files"][$i][ori_name]?>"><?=$arrBoardArticle["files"][$i][ori_name]?></a></td>
						</tr>
					<?
						}
					}?>
