...
<div id="admin-content">
	...
	<form name="form1" method="get" action="<?=$_SERVER[PHP_SELF]?>">
		...
		<div class="page-size">
			<select name="page_size" onchange="document.form1.submit()">
				...
				<option value="6" <?if($arrBoardInfo["list"][0]["scale"]=="6"){echo 'selected="selected"';}?>>6</option>
			</select> 개씩 보기
		</div>
	</form>
	<!--Search Form ED-->
	...
	<table class="listTable">
		...
		<tr>
			...
			<td><?=$categoryTitle?></td>
			<td><?=stripslashes($arrBoardList["list"][$i]["category"])?></td>
			<td class="al">
		...
		<div class="sub_content">
			...
			<div class="content_wrap">
				<ul class="tabmenu">
					...
					<li class="active"><a href="press_list.php" class="active">보도자료 및 자료실</a></li>
					...
					<ul class="press_list">
						...
						if($arrBoardList["list"]["total"] > 0){
							for($i=0; $i < $arrBoardList["list"]["total"]; $i++){
								...
								if($arrBoardList["list"][$i][category] == "보도자료"){
									$cateClass = "";
								}else if($arrBoardList["list"][$i][category] == "관련논문"){
									$cateClass = "thesis";
								}else if($arrBoardList["list"][$i][category] == "해외사례"){
									$cateClass = "case";
								}
						?>
						<li>
							<a href="<?=$_SERVER[PHP_SELF]?>?boardid=<?=$arrBoardInfo["list"][0]["boardid"]?>&mode=view&idx=<?=$arrBoardList["list"][$i][idx]?>&sk=<?=$_GET[sk]?>&sw=<?=$_GET[sw]?>&offset=<?=$_GET[offset]?>&category=<?=$_GET[category]?>">
								<span class="cate <?=$cateClass?>"><?=$arrBoardList["list"][$i][category]?></span>
								<p class="title"><?=$arrBoardList["list"][$i][subject]?></p>
								<p class="desc"><?=text_cut(strip_tags($arrBoardList["list"][$i][contents]),50)?></p>
								<p class="date"><?=str_replace("-",".",$arrDate[0])?></p>
							</a>
						</li>
						<?
							}
						...
						<!-- <li>
							<a href="./notice_view.php">
								<span class="cate thesis">관련논문</span>
								<p class="title">지플러스생명과학, 한국식물생명 공학회</p>
								<p class="desc">지플러스생명과학, 식물기반 바이오의약품 및 종자 품질보증 위해 ETS(Excellence Through Stewardship) 가입</p>
								<p class="date">2021.09.09</p>
							</a>
						</li>
						<li>
							<a href="./notice_view.php">
								<span class="cate case">해외사례</span>
								<p class="title">지플러스생명과학, 한국식물생명 공학회</p>
								<p class="desc">지플러스생명과학, 식물기반 바이오의약품 및 종자 품질보증 위해 ETS(Excellence Through Stewardship) 가입</p>
								<p class="date">2021.09.09</p>
							</a>
