...
<div id="admin-content">
	...
	<form name="form1" method="get" action="<?=$_SERVER[PHP_SELF]?>">
		...
		<div class="page-size">
			<select name="page_size" onchange="document.form1.submit()">
				...
				<option value="8" <?if($arrBoardInfo["list"][0]["scale"]=="8"){echo 'selected="selected"';}?>>8</option>
			</select> 개씩 보기
		</div>
	</form>
	<!--Search Form ED-->
	...
	<table class="listTable">
		...
		<tr>
			...
			<td><?=$categoryTitle?></td>
			<td><img src="<?=$imgsrc[$i]?>" style="max-width:100%;max-height:100px;"></td>
			<td class="al">
			<?=text_cut(stripslashes($arrBoardList["list"][$i][subject]),$arrBoardInfo["list"][0][subjectcut])?>(<?=stripslashes($arrBoardList["list"][$i]["etc_1"])?>)</td>
...
<?}else{###################################################### 사용자 페이지 ######################################################?>
	...
	<div class="sub_content_wrap">
		<?php include "../include/_snb_product.php" ?>
		
		<div class="sub_content">
			<div class="title_wrap">
				<h3 class="page_title">제품 안내</h3>
			</div>
			
			<div class="content_wrap">
				
				<div class="board_list">
					<p class="total line">총<span class="fc_point"><?=number_format($arrBoardList["total"])?></span>건</p>
					
					<ul class="gallery_list product">
						<?
						if($arrBoardList["list"]["total"] > 0){
							for($i=0; $i < $arrBoardList["list"]["total"]; $i++){
								$imgsrc[$i] = "/uploaded/board/".$arrBoardInfo["list"][0]["boardid"]."/".$arrBoardList["list"][$i][re_name];
								if(!$arrBoardList["list"][$i][re_name]){$imgsrc[$i] = "/pub/image/sample/img_tmp_product01.png";}
						?>
						<li>
							<a href="javascript:popup.show('popPatent_<?=$arrBoardList["list"][$i][idx]?>');">
								<div class="thumb"><img src="<?=$imgsrc[$i]?>" alt=""></div>
								<div class="info_wrap">
									<p class="tit"><strong><?=$arrBoardList["list"][$i][subject]?></strong>(<?=$arrBoardList["list"][$i][etc_1]?>)</p>
									<p class="desc"><?=stripslashes($arrBoardList["list"][$i][contents])?></p>
								</div>
							</a>
						</li>
						<?
	...
	if($arrBoardList["list"]["total"] > 0){
		for($i=0; $i < $arrBoardList["list"]["total"]; $i++){
			$imgsrc[$i] = "/uploaded/board/".$arrBoardInfo["list"][0]["boardid"]."/".$arrBoardList["list"][$i][re_name];
			if(!$arrBoardList["list"][$i][re_name]){$imgsrc[$i] = "/pub/image/sample/img_tmp_patent01.jpg";}
	?>
	<!--popup -->
	<div class="popup" id="popPatent_<?=$arrBoardList["list"][$i][idx]?>">
		<div class="pop_wrap">
			<h1 class="pop_title">제품 상세</h1>
