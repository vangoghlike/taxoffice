...
<div id="admin-content">
	...
	<table class="listTable">
		...
		<tr>
			...
			<td><a href="<?=$_SERVER[PHP_SELF]?>?boardid=<?=$arrBoardInfo["list"][0]["boardid"]?>&mode=view&idx=<?=$arrBoardList["list"][$i][idx]?>&sk=<?=$_GET[sk]?>&sw=<?=$_GET[sw]?>&offset=<?=$_GET[offset]?>&category=<?=$_GET[category]?>">
			<?=$arrBoardList["list"][$i][etc_1]?></a></td>
	...
	<div class="blist">
		<table cellpadding="0" cellspacing="0" summary="게시판입니다.">
			...
			if($arrBoardList["list"]["total"] > 0){
				for($i=0; $i < $arrBoardList["list"]["total"]; $i++){
					$categoryTitle = $arrBoardList["total"]-$i-$_GET[offset];
					$arrCatNoList = getCategoryFree(" AND cat_board_id = '".$arrBoardList["list"][$i]["board_id"]."'");
					if ( $arrCatNoList["list"][0]["cat_no"] ) {
			?>
				<tr class="">
                    ...
                    <?php
                        $_selim_server_url = $_SERVER[PHP_SELF];
                        switch ($arrBoardList["list"][$i]["board_id"]) {
                            case 'hanpage':
                                $_selim_server_url = 'http://www.taxoffice.co.kr/sub/?cat_no=21';
                                ?>
                                href="<?=$_selim_server_url?>&boardid=<?=$arrBoardList["list"][$i]["board_id"]?>&mode=view&sidx=<?=$arrBoardList["list"][$i]["idx"]?>&sk=<?=$_GET[sk]?>&sw=<?=$_GET[sw]?>&offset=<?=$_GET[offset]?>&category=<?=$_GET[category]?>"
                                <?php
                                break;
                            case 'en_latest':
                                $_selim_server_url = 'http://www.etaxoffice.co.kr/eng/sub/';
                                ?>
                                href="<?=$_selim_server_url?>?cat_no=<?=$arrCatNoList["list"][0]["cat_no"]?>&boardid=<?=$arrBoardList["list"][$i]["board_id"]?>&mode=view&sidx=<?=$arrBoardList["list"][$i]["idx"]?>&sk=<?=$_GET[sk]?>&sw=<?=$_GET[sw]?>&offset=<?=$_GET[offset]?>&category=<?=$_GET[category]?>"
                                <?php
                                break;
                            default :
                                $_selim_server_url = $_SERVER[PHP_SELF];
					...
					<?}else{?>
					href="javascript:alert('들어갈 수 없는 페이지 입니다.')"
					<?}?>
					><?=text_cut($arrBoardList["list"][$i][subject],50)?></a> <?=$newImage?></td>
					<td class="td_file"> <?=substr($arrBoardList["list"][$i]['reg_date'],0,10)?></td>
				</tr>
			<?
                    }
				}
			...
			?>
				<tr class="">
					<td class="td_num" colspan="3">등록된 데이터가 없습니다</td>
				</tr>
			<?
			}
			?>
