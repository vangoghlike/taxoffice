		...
		<table class="listTable" cellpadding="0" cellspacing="0" summary="게시판입니다.">
			...
			<thead>
				<tr>
					...
					<th scope="col">작성자<button class="btn_ord up" onclick="ordSort('name')"></button><button class="btn_ord down" onclick="ordSort('name desc')"></button></th>
			...
			?>
				<tr <?=$TrClass?>>
					...
					<?if($arrBoardList["list"][$i]["board_id"] != "faq"){?>
					...
					<?}else{?>
					<td class="subject"><a href="<?=$_SERVER[PHP_SELF]?>?cat_no=105&boardid=<?=$arrBoardList["list"][$i]["board_id"]?>&mode=view&idx=<?=$arrBoardList["list"][$i][idx]?>&sk=<?=$_GET[sk]?>&sw=<?=$_GET[sw]?>&offset=<?=$_GET[offset]?>&category=<?=$_GET[category]?>" class="" data-idno="30253"><?=text_cut($arrBoardList["list"][$i][subject],50)?></a> <?=$newImage?></td>
					<?}?>
