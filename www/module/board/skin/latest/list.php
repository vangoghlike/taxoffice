		...
		<table cellpadding="0" cellspacing="0" summary="게시판입니다.">
			...
			?>
				<tr class="">
					...
					href="<?=$_SERVER[PHP_SELF]?>?cat_no=<?=$arrCatNoList["list"][0]["cat_no"]?>&boardid=<?=$arrBoardList["list"][$i]["board_id"]?>&mode=view&sidx=<?=$arrBoardList["list"][$i]["idx"]?>&sk=<?=$_GET[sk]?>&sw=<?=$_GET[sw]?>&offset=<?=$_GET[offset]?>&category=<?=$_GET[category]?>"
					<?}else{?>
