...
<?}else{###################################################### 사용자 페이지 ######################################################?>
<?
	$arrDate = explode(" ",$arrBoardArticle["list"][0][wdate]);
?>	
			...
			<div class="content_wrap">
				<div class="board_view">
					...
					<div class="bd_header">
						...
						<dl class="info">
							<dt>작성일</dt>
							<dd><?=str_replace("-",".",$arrDate[0])?></dd>
							
							<dt>조회수</dt>
							<dd><?=number_format($arrBoardArticle["list"][0][hit])?></dd>
							...
							<dd>
								...
								?>
								<a href="<?=$imgsrc[$i]?>" download="<?=$arrBoardArticle["files"][$i][ori_name]?>"><?=$arrBoardArticle["files"][$i][ori_name]?></a>
								<?}}?>
							</dd>
						</dl>
					</div>
						...
						<div class="content">
							...
							for($i=0;$i<$arrBoardArticle["total_files"];$i++){
								if(substr($arrBoardArticle["files"][$i][re_name],0,2) == "v_") {
									$imgsrc[$i] = "/uploaded/board/".$arrBoardInfo["list"][0]["boardid"]."/".$arrBoardArticle["files"][$i][re_name];
									$arrFtype = explode("/",$arrBoardArticle["files"][$i]["type"]);
									if($arrFtype[0] == "video"){
							?>
							<div class="video">
								<video src="<?=$imgsrc[$i]?>" width="100%" height="520" controls></video>
							</div>
							<?}else{?>
							<div class="video">
							...
							<?if($arrBoardArticle["list"][0][etc_1] != ""){?>
							<div class="video">
								<iframe src="<?=$arrBoardArticle["list"][0][etc_1]?>" width="100%" height="520"></iframe>
							</div>
							<?}?>
					...
					<ul class="bd_nav">
						<li>
							<div class="nav_prev">이전글</div>
							<div class="tit"><?if($arrBoardArticle["prev"]["idx"] !=0):?><a href="<?=$_SERVER[PHP_SELF]?>?boardid=<?=$arrBoardInfo["list"][0]["boardid"]?>&mode=view&idx=<?=$arrBoardArticle["prev"]["idx"]?>&category=<?=$_GET[category]?>" title="<?=$arrBoardArticle["prev"]["subject"]?>" ><?=text_cut($arrBoardArticle["prev"]["subject"],$arrBoardInfo["list"][0][subjectcut])?></a><?else:?><a href="javascript:void(0);" >이전글이 없습니다.</a><?endif;?></div>
						</li>
						<li>
							<div class="nav_next">다음글</div>
							<div class="tit"><?if($arrBoardArticle["next"]["idx"] !=0):?><a href="<?=$_SERVER[PHP_SELF]?>?boardid=<?=$arrBoardInfo["list"][0]["boardid"]?>&mode=view&idx=<?=$arrBoardArticle["next"]["idx"]?>&category=<?=$_GET[category]?>" title="<?=$arrBoardArticle["next"]["subject"]?>" ><?=text_cut($arrBoardArticle["next"]["subject"],$arrBoardInfo["list"][0][subjectcut])?></a><?else:?><a href="javascript:void(0);" >다음글이 없습니다.</a><?endif;?></div>
						</li>
					</ul>

					<div class="btn_wrap">
						<a href="<?=$_SERVER["PHP_SELF"]?>" class="btn">목록보기</a>
					</div>
