...
<div id="admin-content">
	...
	<table class="writeTable">
		...
		for($i=0;$i<$arrBoardArticle["total_files"];$i++){
			...
			if(substr($arrBoardArticle["files"][$i][re_name],0,2) == "v_") {
				$listvideo = "Y";
				$v_num = $i;
			}
		}
		...
		<tr>
			<th>유튜브 링크</th>
			<td colspan="3">
				<input type="text" class="txt req" style="width:70%" name="etc_1" id="etc_1" value="<?=stripslashes($arrBoardArticle["list"][0][etc_1])?>" />
			</td>
		</tr>
		<tr>
			<th>썸네일 이미지</th>
		...
		<!-- <tr>
			...
			<td colspan="3">
				<? if($listvideo == "Y") {?>
				<input type="checkbox" name="filedel[]" value="<?=$arrBoardArticle["files"][$v_num][idx]?>" id="filedel_99"><label for="filedel_99">삭제</label> : <?=$arrBoardArticle["files"][$v_num][ori_name]?>
				<?}else{?>
				<input name="upfiles[]" type="file" class="type01"  style="width: 400px;"><input type="hidden" name="memo_name[]" value="v"> 최적사이즈: 230px * 144px
				<?}?>
			</td>
		</tr> -->
