...
<div id="admin-content">
	...
	<table class="listTable">
		...
		<tbody>
		<?
		var_dump($arrBoardList["list"][0]["total_files"]);
		...
		<div class="sub_content">
			...
			<div class="content_wrap">
				<ul class="tabmenu">
					...
					<li class="active"><a href="community_list.php">커뮤니티</a></li>
				</ul>
				...
				<div class="board_list">
					...
					if($arrBoardList["list"]["total"] > 0){
						for($i=0; $i < $arrBoardList["list"]["total"]; $i++){
							...
							if($imgsrc[$i] == ""){$imgsrc[$i] = "/pub/image/sample/img_tmp_gallery01.jpg";}
					...
					?>
						<li>
							<a href="<?=$_SERVER[PHP_SELF]?>?boardid=<?=$arrBoardInfo["list"][0]["boardid"]?>&mode=view&idx=<?=$arrBoardList["list"][$i][idx]?>&sk=<?=$_GET[sk]?>&sw=<?=$_GET[sw]?>&offset=<?=$_GET[offset]?>&category=<?=$_GET[category]?>">
								<div class="thumb video"><img src="<?=$imgsrc[$i]?>" alt=""></div>
								<div class="info_wrap">
