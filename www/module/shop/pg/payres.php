...
include_once $_SERVER[DOCUMENT_ROOT] . "/module/shop/shop.lib.php";
	...
	$configPath = $_SERVER[DOCUMENT_ROOT]."/module/shop/pg/lgdacom"; //LG텔레콤에서 제공한 환경파일("/conf/lgdacom.conf,/conf/mall.conf") 위치 지정. 
        ...
        if( "0000" == $xpay->Response_Code() ) {
            ...
            //최종결제요청 결과 성공 DB처리 실패시 Rollback 처리
          	...
          	if( !$isDBOK ) {
                ...
                if( "0000" == $xpay->Response_Code() ) {
                  	jsGo("/shop.php?goPage=Cart","parent","자동취소가 정상적으로 완료 되었습니다.");
                }else{
          			jsGo("/shop.php?goPage=Cart","parent","자동취소가 정상적으로 처리되지 않았습니다.");
                }
          	}     
			...
			jsGo("/shop.php?goPage=Thanks&order_no=".$orderid,"parent","");
          	...
          	//최종결제요청 결과 실패 DB처리
         	echo "<script>alert('결제연동 실패하였습니다.'); history.go(-1);</script>";     
	        echo "TX Response_code = " . $xpay->Response_Code() . "<br>";
	        echo "TX Response_msg = " . $xpay->Response_Msg() . "<p>";
